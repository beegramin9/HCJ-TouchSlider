...
if (window.PointerEvent) {
    ...
    window.addEventListener('mouseleave', handleGestureEnd);
    window.addEventListener('contextmenu', handleContextMenu);
    //* Touch Event
    // false는 event propagation 방지, event 중복 방지라고 생각하면 편하다
    window.addEventListener('touchstart', handleGestureStart, false);
    window.addEventListener('touchmove', handleGestureMove, false);
